...
Idling counts as legitimate only when the IRIS sweep driver is present, ACTIVE and has a known future `next_run_at`, or when that driver is present and ACTIVE and a declared hold has a resume condition that can actually be met. A PAUSED, `unknown_status` or otherwise non-ACTIVE driver never makes idling legitimate, whatever its `next_run_at` and whatever hold is declared. Another driver's future run, such as Radar's, does not make a stopped IRIS loop legitimate.
...
- A refusal is never silent. When `finalize --dir` refuses a draft or receipt (for example the allocation was exceeded with no reason, the draft step failed and left no draft, the draft's allocation is not a dict (`invalid_allocation`) or holds a non-numeric budget value (`allocation_not_numeric:<key>`), its coverage is missing (`invalid_coverage`), or settling it raised any other error (`draft_not_settleable:<ExceptionType>`)), it still writes that day's receipt with outcome `failed`, the refusal in `refusal_reasons` and in the coverage gaps, the allocation (minus any non-numeric budget value) and `used` it could salvage, and every candidate `held` with reason `receipt_refused`. It writes no proof, outbox or evidence-ledger file and exits 5. The day then reads as ran-and-failed: a second plan that day gets `already_ran_today`, it is not a missed day, and the next pass lists it in `gap_dates`. An unexplained excess is recorded as `allocation_exceeded` true with a reason starting `not stated`. Stage 4 runs finalize even when the draft step failed.
...
- Allocation accounting: `finalize_receipt` compares `used` with the allocation (`model_launches`, `wall_seconds` against `wall_minutes` × 60, `source_opens`). A budget value (`wall_minutes`, `model_launches`, `source_opens`, `deep_candidates`, `present_max`) that is present but not a non-negative number is refused as `allocation_not_numeric:<key>`, never skipped. Any excess needs a non-empty `allocation_exceeded_reason`, or the receipt is refused. The receipt always records `allocation_exceeded` as true or false, computed and never taken from the input. The runner supplies an explicit reason when its watchdog fires; that watchdog (`DAILY_SCAN_INTEL_TIMEOUT`, default 1440 s) is set one minute under `wall_minutes` so it fires before the allocation is exceeded. Otherwise the model's own `allocation_exceeded_reason` is used, prefixed `model:`. The pass's wall clock (`started_at`) starts after the novelty corpus is built; the build is timed separately as `corpus_build_seconds` and never counts against `wall_minutes`.
...
- Evidence identities. Every identity is first unwrapped (surrounding brackets, parentheses, quotes, backticks and angle brackets) and trimmed (trailing `,.;:/!?…`, quotes and backticks, and a closing bracket only while it is unbalanced, so `…/Foo_(bar)` keeps its own). For internal evidence an identity is the first whitespace-delimited token of each `ref`, lowercased, without a leading `./` (`board:8c7450d0`). A path-like token (one with a `/`, a leading `~` or a file extension) also loses a `#anchor` and a `:<line>` suffix (`:12`, `:12:5`, `:12-20`); `board:142` is not path-like and keeps its `:142`. For external evidence it is the optional `url` when present, and otherwise the whole source (not its first token) lowercased with whitespace collapsed, cut to 80 characters and trimmed. Any URL (an internal first token, an external `url`, or a source that is a URL) loses its query string, fragment, `;params` and userinfo; `http` and `https` both read as `https`; a leading `www.` and the default ports `:80` and `:443` are dropped; the path is percent-decoded; and the whole is lowercased and trimmed, trailing `/` included. Counterevidence, claims, ref descriptions, observation times and wording are never identities. `evidence_revision` hashes the set of identities; each identity's docket token is `short_token` of it (short refs as-is, anything else `ref-<hash>`).
...
- That suppression catches every resubmission. It is identity-based only. A new `dedupe_key` with reworded text is a new opportunity and passes suppression entirely; only the independent challenge and the novelty floor (a crude word-overlap check against earlier docket packets and outbox files) stand in its way. Within one `dedupe_key`, citing one identity not seen before (a new or differently spelled ref token, a different URL path, a renamed source) makes it `eligible_changed_evidence` even when nothing material changed; the normalisation covers only the spelling variants listed under Evidence identities above, and anything else, such as a redirect, a mirror or a shortened link, is a different identity. Code enforces only "at least one evidence identity is new". Materiality is judged by the pass, the challenger and Anthony.
...
- That a pass inside its budget always settles when the host stalls. The stage 4 watchdog (`DAILY_SCAN_INTEL_TIMEOUT`, default 1440 s) fires 60 s before the 25-minute `wall_minutes` allocation and then supplies the allocation-exceeded reason itself, so ordinary model calls plus the host's own seconds settle within budget. The margin is not a guarantee: if the Mac sleeps mid-pass (`sleep` does not count system sleep but `wall_seconds` does), or the host steps around the model calls take more than 60 s, `wall_seconds` can pass the allocation with no watchdog reason. Finalize then refuses, and the day is recorded as a `failed` receipt rather than an accepted one. An override of `DAILY_SCAN_INTEL_TIMEOUT` at or above 1500 s reopens the gap.
